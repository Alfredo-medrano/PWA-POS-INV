...
import {
  ...
  CheckCircle, Plus, Edit, XCircle, Download, RefreshCw, Wifi, WifiOff
} from "lucide-react";
...
export default function Config({ dteConnected, setDteConnected }: { dteConnected: boolean; setDteConnected: (v: boolean) => void }) {
  ...
  const [url, setUrl] = useState("https://api.mi-sistema-dte.com");
        ...
        <div className="flex-1 w-full">
          {sec === "dte" && (
            <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-6 space-y-5">
              <div><h2 className="font-black text-[#0F172A]">Conexión con Sistema de Facturación Electrónica</h2><p className="text-xs text-[#94A3B8] mt-0.5">Configura el acceso a tu sistema DTE externo</p></div>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#94A3B8] mt-1.5">Cargado automáticamente desde el sistema DTE</p>
                </div>
              </div>
              ...
              <div className={`rounded-xl p-4 flex items-start gap-3 ring-1 ${dteConnected ? "bg-emerald-50 ring-emerald-200" : "bg-red-50 ring-red-200"}`}>
                ...
                <div>
                  ...
                  {dteConnected ? (
                    <>
                      <p className="text-xs text-emerald-600 mt-0.5">Último ping: hace 2 min</p>
                      <p className="text-xs text-emerald-600">Emisor: Mi Tienda S.A. de C.V. · NIT: 0614… · Producción</p>
                    </>
                  ) : (
              ...
              {testRes === "ok" && (
                <div className="flex items-center gap-2 bg-emerald-50 text-emerald-700 p-3.5 rounded-xl text-xs font-semibold ring-1 ring-emerald-200"><CheckCircle size={14} />Conexión exitosa — Mi Tienda S.A. de C.V. | Producción</div>
              )}
            </div>
          )}
          ...
          {sec === "usuarios" && (
            <div className="space-y-4">
              <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-5">
                <div className="flex items-center justify-between mb-4"><h2 className="font-black text-[#0F172A]">Lista de usuarios</h2><Btn v="secondary" sz="sm"><Plus size={13} />Agregar usuario</Btn></div>
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-[#E2E8F0]"><th className="text-left pb-2 text-xs font-bold text-[#94A3B8]">Usuario</th><th className="text-left pb-2 text-xs font-bold text-[#94A3B8]">Rol</th><th className="text-left pb-2 text-xs font-bold text-[#94A3B8]">Estado</th><th className="pb-2"></th></tr></thead>
                  <tbody className="divide-y divide-[#F8FAFC]">
                    {[{ n: "Carlos Gomez (tú)", r: "Administrador", e: "Activo" }, { n: "Ana Martinez", r: "Cajero", e: "Activo" }].map(u => (
                      <tr key={u.n} className="hover:bg-slate-50"><td className="py-2.5 font-semibold text-[#0F172A]">{u.n}</td><td className="py-2.5 text-[#64748B]">{u.r}</td><td className="py-2.5"><Badge color="green">{u.e}</Badge></td><td className="py-2.5 text-right"><button className="text-[#94A3B8] hover:text-[#64748B]"><Edit size={13} /></button></td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
              <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-5 overflow-x-auto">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-[#F8FAFC]">
                    {PERMS.map((p, i) => (
                      <tr key={p}><td className="py-2.5 pr-4 text-[#64748B] text-sm">{p}</td>
          ...
          {sec === "plan" && (
            <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-6 space-y-5">
              ...
              <div className="bg-gradient-to-br from-[#EEF2FF] to-[#E0E7FF] rounded-2xl p-5 flex items-center justify-between ring-1 ring-[#1B4FD8]/20">
                <div><div className="flex items-center gap-2.5 mb-1"><p className="font-black text-[#0F172A] text-xl">Plan Estándar</p><Badge color="blue">ESTÁNDAR</Badge></div><p className="text-sm text-[#64748B]">Hasta 3 usuarios · Inventario ilimitado · Reportes básicos</p></div>
